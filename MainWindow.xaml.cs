...
public partial class MainWindow : Window
{
    ...
    private void RenderGroups()
    {
        ...
        foreach(var account in accounts.OrderByDescending(account=>rows.Count(row=>row.AccountId==account.Id)))
        {
            ...
            if(matches.Count>0)
            {
                ...
                foreach(var (title,path,width) in new (string,string,double)[]{("我方呼号","OwnCall",105),("台站","StationName",145),("日期 UTC","Date",100),("时间 UTC","Time",90),("频率 / 卫星","Channel",175),("模式","Mode",70),("发 / 收报告","Reports",110),("纸卡已寄","Qsl",85),("LoTW 是否确认","Lotw",130)})
    ...
    private void Export(object sender,RoutedEventArgs args)
    {
        ...
        try
        {
            var lines=new List<string>{"我方呼号,台站,友台呼号,日期UTC,时间UTC,频率或卫星,模式,发收报告,纸卡已寄,LoTW是否确认,国家,邮箱,地址,资料来源"};
            lines.AddRange(rows.Select(row=>string.Join(",",new[]{row.OwnCall,row.StationName,currentCall,row.Date,row.Time,row.Channel,row.Mode,row.Reports,row.Qsl,row.Lotw,Country.Text,Email.Text,Address.Text,LookupStatus.Text}.Select(Csv))));
